...
@Composable
// TODO: NOT WORKING
fun ColumnScope.Noise(modifier: Modifier = Modifier, f: Float) {
    val shadderCode = """
uniform float uTime;
...
half4 main(vec2 fragCoord) {
    ...
    // Noise grain
    float n = hash(uv*500.0+uTouch[0]); // scale + animate
    img.r -= sin(uv.x*20.0)+n;
    img.g -= sin(uv.y*20.0)+n;
    img.b -= sin(uTime)-n;
//    img.rgb += (n - uv.x) * uv.y; // subtle grain
